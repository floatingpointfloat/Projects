...
class QuadNode:
    ...
    def __init__(self, x, y, width, height, depth=0):
        ...
        self.sw = None
        self.se = None
        self.entities = []  # Store points if this node is a leaf 
    ...
    def insert_to_children(self, x, y, idx): #try to insert the point into the children - return True if successful, False if not
        child = self.get_child(x, y)
        return child.insert(x, y, idx)
    
    def insert(self, x, y, idx): #insert a point into the quadtree, return True if successful, False if the point is outside the bounds of this node or if the insertion simply failed for some reason
        ...
        if self.nw is None: #if this node is a leaf, try to insert the point here
            if len(self.entities) < QuadNode.CAPACITY or self.depth >= QuadNode.MAX_DEPTH or self.width <= QuadNode.MIN_SIZE or self.height <= QuadNode.MIN_SIZE: #if there is still capacity or we have reached max depth, insert the point here
                self.entities.append((x, y, idx))
                return True
            else: #otherwise, we need to subdivide and redistribute the points
                self.subdivide()
                for px, py, old_idx in self.entities:
                    self.insert_to_children(px, py, old_idx)
                self.entities.clear() #clear the points from this node since they are now stored in the children
                return self.insert_to_children(x, y, idx) #try to insert the new point into the children as well
        else: #if this node is not a leaf, try to insert the point into the children
            return self.insert_to_children(x, y, idx)
    
    def query(self, x, y, found_entities, search_range_sq=None): #search for points within search_range of (x, y) and add them to found_points - maybe for collision or stuff like that
        if search_range_sq is None: #failsafe
            return 
        
        if not self.intersects_circle(x, y, search_range_sq): #if the search range does not intersect with this node, return
            return
            
        for px, py, idx in self.entities: #check the points in this node - if they are within the search range, add them to found_points
            dx = px - x
            dy = py - y
            if dx * dx + dy * dy <= search_range_sq:
                found_entities.append((px, py, idx))
        ...
        if self.nw is not None: #if this node has children, check them as well - also loop unrolling for performance since we know there are only 4 children
            self.nw.query(x, y, found_entities, search_range_sq)
            self.ne.query(x, y, found_entities, search_range_sq)
            self.sw.query(x, y, found_entities, search_range_sq)
            self.se.query(x, y, found_entities, search_range_sq)
...
class QuadTree:
    ...
    def insert(self, x, y, idx):
        return self.root.insert(x, y, idx)
    ...
    def query(self, x, y, search_range):
        ...
        self.root.query(x, y, found_entities, search_range_sq)
