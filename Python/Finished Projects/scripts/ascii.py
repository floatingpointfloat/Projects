...
def resize_image(image, new_width):
    ...
    ratio = (
        height / width
    )  # find the ratio to allow the height to be adjusted according to the new_width
...
def grayscale(image):
    grayscale_image = image.convert(
        "L"
    )  # L is the intensity of the conversion - L for grayscale :)
...
def main(new_width=160):
    while True:
        ...
        path = input("Enter a valid pathname to the desired image: ").replace('"', "")
        try:
        ...
        ascii_image = "\n".join(
            ...
            for i in range(0, pixels_count, new_width)
        )
