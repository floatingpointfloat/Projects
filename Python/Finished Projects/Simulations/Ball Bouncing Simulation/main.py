...
class Ball:
    __slots__ = ["position", "velocity", "acceleration", "radius", "color", "mass"]
    ...
    def update(self):
        ...
        if dist + self.radius >= 500 and self.velocity.dot(normal) > 0:
            self.velocity = (
                self.velocity.reflect(normal) * 0.97
            )  # Dampen the velocity to simulate energy loss
...
while True:
    ...
    for _ in range(3):
        for i in range(len(obj)):
            for j in range(i + 1, len(obj)):
                if (
                    obj[i].position.distance_to(obj[j].position)
    ...
    pygame.display.set_caption(
        f"Bouncing Ball Simulation | Bodies: {len(obj)} | FPS: {round(fps, 1)}"
    )
